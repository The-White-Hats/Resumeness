  ...
  return (
    <div className="preview">
      ...
      <div className="main">
        ...
        <div className={"experience " + ((experienceArr.length === 0) ? "hide" : '')}>
          <h2>Experience</h2>
          {experienceArr.map((experience) => {
            return (
              <div key={experience.id}>
                <h3>{experience.jobTitle}, {experience.company}</h3>
                <h3><i>{experience.startDate} - {experience.endDate}</i></h3>
                <p>{experience.jobDescription}</p>
                <h3 className={(experience.achievements.length === 0) ? "hide" : ''}>Achievements</h3>
                <ul>
        ...
        <div className={"education " + ((educationArr.length === 0) ? "hide" : '')}>
          <h2>Education</h2>
          {educationArr.map((education) => {
            return (
              <div key={education.id}>
                <h3>{education.degree}, {education.studyField}</h3>
                <h3>{education.school}</h3> 
                <h3><i>{education.graduationDate}</i></h3>
                <h3 className={(education.awards.length === 0) ? "hide" : ''}>Awards</h3>
                <ul>
        ...
        <div className={"certifications " + ((certificationArr.length === 0) ? "hide" : '')}>
          <h2>Certifications</h2>
          {certificationArr.map((certification) => {
            return (
              <div key={certification.id}>
                <h3>{certification.certification}</h3>
                <h3><i>{certification.date}</i></h3>
              </div>
            );
          })}
        </div>
        <div className={"volunteering " + ((volunteeringArr.length === 0) ? "hide" : '')}>
          <h2>Volunteering</h2>
          {volunteeringArr.map((volunteering) => {
            return (
              <div key={volunteering.id}>
                <h3>{volunteering.jobTitle}, {volunteering.company}</h3>
                <h3><i>{volunteering.startDate} - {volunteering.endDate}</i></h3>
                <p>{volunteering.jobDescription}</p>
                <h3 className={(volunteering.achievements.length === 0) ? "hide" : ''}>Achievements</h3>
                <ul>
